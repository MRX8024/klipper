# Code for handling the kinematics of hybrid linear delta robots
#
# Copyright (C) // modified
#
...
class HybridDeltaKinematics:
    def __init__(self, toolhead, config):
        ...
        rail_c = stepper.LookupMultiRail(
            ...
            default_position_endstop=a_endstop)
        # Allow pin multi use for hybrid "z" axis endstop
        z_endstop = stepper_configs[3].get('endstop_pin') 
        ppins = config.get_printer().lookup_object('pins')
        ppins.allow_multi_use_pin(z_endstop)
        ...
        self.abs_endstops = [(rail.get_homing_info().position_endstop
                             ...
                             for rail, arm2 in zip(self.delta_rails, self.arm2)]
        ...
        for r, a, t in zip(self.delta_rails, self.arm2, self.towers):
    ...
    def calc_position(self, stepper_positions):
        spos = [stepper_positions[rail.get_name()] for rail in self.delta_rails]
...
def load_kinematics(toolhead, config):
    return HybridDeltaKinematics(toolhead, config)
